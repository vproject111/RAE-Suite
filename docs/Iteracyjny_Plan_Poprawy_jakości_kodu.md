...
Każdy etap będzie wdrażany jako osobne wydanie SemVer 2.0.0. Ze względu na usunięcie mocków i wartości domyślnych z bramki jakości, Iteracja 1 rozpocznie główny cykl wydawniczy `3.6.1-rc.1`.

---

### 1. Iteracja 1: Deterministic Core & Fixes (Wydanie: `3.6.1-rc.1`)
...
### 2. Iteracja 2: Static Analysis & Security Gateways (Wydanie: `3.7.0`)
*   **Skaner Trivy:**
...
### 3. Iteracja 3: Advanced Verification & API Fuzzing (Wydanie: `3.8.0`)
...
### 4. Iteracja 4: Dependency Hygiene & Secure Supply Chain (Wydanie: `3.9.0`)
...
### 5. Iteracja 5: Enterprise Hardening & Infrastructure Optimizations (Wydanie: `3.10.0`)
*   **Szyfrowanie i Bezpieczeństwo Tokenów (SonarQube):**
    *   Zabezpieczenie `SONARQUBE_TOKEN` w żądaniach MCP i webhookach za pomocą mechanizmu szyfrowania (integracja z menedżerem sekretów Vault lub RAE Enterprise Bridge) zamiast przesyłania go w postaci jawnej.
*   **Weryfikacja Pochodzenia Zależności (Sigstore / PyPI):**
    *   Rozszerzenie konfiguracji Renovate o automatyczną weryfikację podpisów kryptograficznych pobieranych paczek z PyPI przy użyciu wtyczek Sigstore dla języka Python w celu ochrony przed atakami typu *dependency confusion* oraz skażeniem łańcucha dostaw.
*   **Lokalny Cache Baz Podatności (Trivy-DB Proxy):**
    *   Wdrożenie lokalnego serwera proxy dla bazy luk Trivy (`trivy-db`) w infrastrukturze kontenerowej w celu skrócenia czasu audytów statycznych, optymalizacji transferu i zapewnienia pełnego wsparcia dla środowisk odciętych od internetu (air-gapped).
