# RAE Autonomy Evolution & Self-Healing Blueprint (v6.2)
...
Wdrażamy paradygmat **"Zero Uncontrolled Action"** – pełnej samodzielności w granicach zdefiniowanych reguł bezpieczeństwa, klasyfikacji ryzyka oraz kryptograficznego dowodu decyzji w myśl zasady: **No Evidence, No Autonomy**.

---

## 📊 1. Analiza Dojrzałości Modułów (Stan Obecny v5.0)

| Moduł RAE | Poziom Autonomii | Główne Ograniczenie w v5.0 | Kierunek Ewolucyjny (Klasa Autonomii 6.2) |
| :--- | :--- | :--- | :--- |
| **`rae-core`** | Średni-Wysoki | Zmiana długości wektora (np. 384d $\rightarrow$ 768d) jest traktowana jako błąd bazy. | **Embedding Profile Registry & Vector Projection Manager:** Obsługa nieskończonej liczby modeli i automatyczne przeliczanie projekcji wektorowych w tle bez przestojów. |
| **`rae-hive`** | Średni | Brak weryfikacji ryzyka (brak klasyfikacji R0-R6) przed wykonaniem. | **Risk-Controlled P2P Task Swarm:** Centralny klasyfikator ryzyka w jądrze, izolacja sandbox/worktree i P2P load balancing. |
| **`rae-quality`** | Wysoki | Złudne zaufanie do zielonych testów wygenerowanym przez LLM. | **Multi-Tier Quality Gate & TestIntegrityGuard:** Weryfikacja mutacyjna, statyczna i architektoniczna chroniąca przed osłabianiem asercji testowych. |
| **`rae-lab`** | Średni | Zmiany guardraili wdrażane na żywo bez trybu cienia. | **Shadow-Mode Failure Mining:** Lab generuje reguły i testuje je w Shadow Mode przed ostatecznym awansowaniem. |
| **`rae-suite`** | Średni-Wysoki | Brak rozróżnienia ról kodu od infrastruktury; ryzyko auto-pusha na produkcję. | **Signed GitOps PR Daemon & Branch Policy:** Automatyczne PR-y z cyfrowo podpisanymi commitami i Evidence Pack, bez pusha na produkcję. |

---

## 🏗️ 2. Architektura RAE Autonomy Kernel

Jądro decyzyjne RAE Autonomy Kernel jest **centralnym modułem sterującym** współdzielonym przez wszystkie elementy suity, co zapobiega niekontrolowanemu działaniu agentów.
...
Wszystkie moduły suity kategoryzują planowane operacje według poniższych klas ryzyka:
...
*   **R5:** Zmiany infrastruktury produkcyjnej, naprawa danych klientów, modyfikacje sekretów i kluczy szyfrujących (Wysokie ryzyko).
*   **R6:** Działania bezwzględnie zabronione (destrukcyjne operacje na danych, naruszanie polityk bezpieczeństwa, próby wyciągnięcia sekretów poza allowlistę). **R6 nigdy nie może zostać wykonany przez system, nawet na bezpośrednie żądanie modelu.**
...
*   **Zasada:** **Memory != Vector**. Wektor to wyłącznie matematyczna projekcja pamięci źródłowej. Dwa modele o tym samym wymiarze (np. 768) mogą mieć zupełnie inną semantykę.
*   **Kontrakt `EmbeddingProfile`:**
    ...
    class EmbeddingProfile:
        ...
        model_hash: str | None     # kryptograficzny hash wag modelu
        tokenizer_hash: str | None # hash konfiguracji tokenizera
        created_at: datetime
        active: bool
    ```
*   **Autonomia:** Zmiana profilu embeddingu nie wywołuje awarii. `VectorProjectionManager` tworzy nową kolekcję (np. `memories__provider__model__dimension`) i w tle wyzwala `Background Indexer Job`. Do czasu osiągnięcia progu pokrycia >95%, system działa w **trybie degraded** (fallback do tekstowego wyszukiwania lub starszej kolekcji).
...
*   **Architektura:** Każde zadanie wywołania kodu generowanego przez model jest uruchamiane wyłącznie w wydzielonym kontenerze Docker oraz w odizolowanym katalogu roboczym (Git Worktree). Żadne modyfikacje kodu nie są dokonywane bezpośrednio na żywej bazie kodu deweloperskiego.
...
*   **TestIntegrityGuard [NEW]:** Nowy, dedykowany moduł dbający o to, by agent nie próbował „naprawić” kodu poprzez osłabianie asercji testowych, kasowanie trudnych testów regresyjnych czy modyfikację plików testowych. Każde pogorszenie jakości lub pokrycia testów automatycznie odrzuca zmianę (`REJECT/QUARANTINE`).

### Krok 6: Closed-Loop Phoenix Refactoring w Sandboxie
*   **Architektura:** Phoenix naprawia kod wyłącznie wewnątrz wydzielonego sandboxa roboczego. Cada próba naprawy jest rekurencyjnie ewaluowana przez dynamiczny Quality Gate. Phoenix ma twarde limity: kosztu (Token budget), czasu (Timeout) oraz prób (Max 5), po których system automatycznie zatrzymuje pętlę i eskaluje problem do człowieka.
...
*   **Autonomia:** Promocja reguły AST z `Candidate Guardrail` do wersji aktywnej następuje po spełnieniu kryteriów:
    *   Wskaźnik False Positive na bazie historycznych replayów logów mieści się poniżej ustalonego progu (np. < 0.1%).
...
*   **Architektura:** Metryki OTEL są zegrane stale z sub-sekundową dokładnością, ale decyzje o dostrojeniu Routera decyzyjnego są podejmowane w bezpiecznych oknach czasowych (30–300 sekund) z zastosowaniem histerezy. Zapobiega to oscylacjom sieciowym i nieprzewidywalnym przełączeniom modeli przy chwilowych wahaniach sieci.
...
Wprowadzamy kardynalną zasadę ekosystemu: **No Evidence, No Autonomy** (Brak dowodów = brak autonomii). Każde działanie systemu musi wygenerować kompletny kryptograficzny pakiet dowodowy (**ISO Evidence Pack**) przed zakończeniem zadania.

Każda autonomiczna akcja rejestruje:
1.  **`goal_id` / `task_id` / `trace_id`:** Śledzenie pochodzenia intencji.
2.  **`risk_class`:** Wynik klasyfikatora ryzyka (R0-R6).
3.  **`policy_decision`:** Zgodność z politykami RAE.
4.  **`sandbox_id` / `worktree_id`:** Identyfikator odizolowanego środowiska wykonania.
5.  **`quality_result`:** Zrzut ocen Quality Gate i TestIntegrityGuard.
6.  **`evidence_pack_uri`:** Ścieżka do zapisanego kryptograficznego pakietu dowodowego.
7.  **`decision_ledger_entry`:** Cyfrowo podpisany SHA-256 wpis w rejestrze Ledger.
8.  **`rollback_plan`:** Plan wycofania zmian (wymagany dla wszystkich operacji klasy R3+).
9.  **`memory_writeback_status`:** Status synchronizacji z warstwą refleksyjną RAE-Memory.

---

## 🛡️ 5. Poprawiona Definicja Sukcesu (DoD) - Autonomy Level 6.2

Miarą sukcesu jest **"Zero Uncontrolled Action"** (Zero Niekontrolowanych Akcji). Kryteria DoD:

*   **Risk Governance:** Każde zadanie posiada formalną klasyfikację ryzyka (R0-R6) z twardą blokadą powyżej klasy R2.
*   **Sterylna Izolacja:** Każda modyfikacja kodu powstaje wyłącznie w kontenerze sandbox i osobnym worktree.
...
*   **Kryptograficzna Audytowalność:** Każda decyzja posiada paczkę dowodów `Evidence Pack` i wpis SHA-256 w rejestrze Ledger.
